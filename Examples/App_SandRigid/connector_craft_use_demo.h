...
class connector_craft_use_demo : public GLApp
{

private:
	connector_craft_use_demo()//这个与类同名的函数，叫//构造函数//，是用系统自动调用的，调用了键盘操作函数
	{
		setKeyboardFunction(connector_craft_use_demo::demoKeyboardFunction);
		createWindow(1024, 768);
	}
	static connector_craft_use_demo* m_instance;//当前类（塔尖类）的静态指针对象

public:
	static connector_craft_use_demo* getInstance()
	{
		if (m_instance == 0)
			m_instance = new connector_craft_use_demo;
		return m_instance;
	}

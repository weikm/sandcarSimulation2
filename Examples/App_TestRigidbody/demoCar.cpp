...
void DemoCar2::build(bool useGPU)//目标碰撞部分代码就在这
{
    ...
    this->addCar(m_car, Vector3f(0.1, /*-0.15*/ 0.90, 0.1), 1, 4 | 8, 2, 4 | 8);//给m_car附上参数，包括碰撞用的参数，沙地的没做成碰撞很可能是参数不全
    ...
    this->addCar(m_car2, Vector3f(0.1, 0.9, -0.7), 4, 1 | 2, 8, 1 | 2);//给m_car2附上一系列参数
...
{//把车PBDCar的一系列参数全部赋值上了，很全面，后四个形参用于碰撞
    ...
    car->chassisCollisionGroup = chassisGroup;//在这赋值，用在哪了？在PBDCar里面有应用，这边赋值就够了。
...
void DemoTankCar::build(bool useGPU)
{
	...
	//下面这两段，是做碰撞的吧
    {
        auto pointset = TypeInfo::cast<PointSet<DataType3f>>(m_car->getChassis()->getTopologyModule());//这个getTopologyModule不用加在PBDCar里，因为是node的成员函数
        if (pointset)
        {
            //Vector3f chaCenter;
            //Vector3f chaSize;

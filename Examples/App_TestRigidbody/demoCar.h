...
class DemoCar2 : public GLApp//用的就是这个。拿这个类和DemoParticleSandMultiRigid2做个对比
{
private:
    DemoCar2() {}
...
public:
    ...
    void build(bool useGPU = true);//核心

    void run() {}

    virtual void advance(Real dt);//里面空的 没代码
...
public:
    ...
    void computeAABB(std::shared_ptr<PointSet<DataType3f>> points, Vector3f& center, Vector3f& halfSize);//用于碰撞

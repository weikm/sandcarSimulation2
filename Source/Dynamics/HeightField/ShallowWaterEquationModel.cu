    ...
    Real               dt)
{
    ...
    if (i < grid_vel_x.size())
    {
        ...
        else
        {
            ...
            Real u = grid_vel_x[i]
                 ...
                 w = 0;
            w += grid_vel_z[ix * (zcount + 1) + iz - 1];
            w += grid_vel_z[ix * (zcount + 1) + iz];
            w += grid_vel_z[(ix + 1) * (zcount + 1) + iz - 1];
            w += grid_vel_z[(ix + 1) * (zcount + 1) + iz];
            w *= 0.25;
    ...
    if (i < grid_vel_z.size())
    {
        ...
        else
        {
            ...
            Real w = grid_vel_z[i]
                         * 0.5
                     + 0.25 * (grid_vel_z[i - 1] + grid_vel_z[i + 1]),
                 u = 0;
            u += grid_vel_x[(ix - 1) * (zcount + 2) + iz];
            u += grid_vel_x[(ix - 1) * (zcount + 2) + iz + 1];
            u += grid_vel_x[ix * (zcount + 2) + iz];
            u += grid_vel_x[ix * (zcount + 2) + iz + 1];
            u *= 0.25;

...
void OrientedBodyDetector::doCollision()
{
	...
	//printf("doCollision刚走起来\n");
	std::cout<<"m_obbs.size"<<m_obbs.size()<<"\n";//m_obbs.size()是0！！！前面add函数没把obb给push进去
    // Update transformed obb position and orientation info.
    for (int i = 0; i < m_obbs.size(); ++i)//这个循环一次都没走！
    {
		//printf("doCollision走起来\n");//这个没输出来！

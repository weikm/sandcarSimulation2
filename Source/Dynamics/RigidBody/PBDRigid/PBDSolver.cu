...
{//啥时候会执行这个程序？
	...
	printf("pos a\n");
    // Resize data.
    if (m_nJoints > 0 &&(m_GPUJoints.isEmpty() || m_GPUJoints.size() < m_nJoints))
    {
		printf("pos 1\n");
		printf("m_GPUJoints.size=%d", m_GPUJoints.size(),"\n");//0
		printf("m_nJoints=%d", m_nJoints,"\n");//1
        m_GPUJoints.resize(m_nJoints);//所以问题就是这一行
		printf("pos 2\n");

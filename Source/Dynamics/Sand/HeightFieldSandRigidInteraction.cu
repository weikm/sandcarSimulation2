...
//调用在楼上
void HeightFieldSandRigidInteraction::advectSubStep(Real dt)//平流分支步，被楼上调用。里面调用了ComputeSingleBody
{
    // Advect sand.
    if (m_sandSolver)
    //if(false)
    ...
    if (m_rigidSolver && m_sandSolver)
    {
        ...
        m_interactSolver->updateBodyAverageVel(dt);//更新刚体速度
        ...
        for (int i = 0; i < nrigid; ++i)
        {
            if (m_interactSolver->collisionValid(m_rigidSolver->getRigidBodys()[i]))
            {
                ...
                m_interactSolver->computeSingleBody(i, dt);//computeSingleBody
    ...
    if (m_sandSolver)
    {
        m_sandSolver->updateVeclocity(dt);//这是更新的沙子速度，在哪更新刚体速度哇？就在上面Line159的 updateBodyAverageVel。
        //m_sandSolver->advection(dt);
        //m_sandSolver->updateSandGridHeight();
...
//init中调用
void HeightFieldSandRigidInteraction::detectLandRigidContacts(PBDSolver* solver, Real dt)//接触检测
{
    if (m_landRigidContactDetector)
    {

...
__device__ inline float d_get_v(gridpoint gp)//在这呢，论文公式4.19，P27
{
    float h  = fmaxf(gp.x, 0.0f);
    float h4 = h * h * h * h;
...
__host__ __device__ inline float4 d_flux_v(float4 gpl, float4 gpr)//这函数就是求公式4.14的通量H
{
    float hl  = fmaxf(gpl.x, 0.0f);
    float hl4 = hl * hl * hl * hl;
    float vl  = sqrtf(2.0f) * hl * gpl.z / (sqrtf(hl4 + fmaxf(hl4, EPSILON)));//速度公式4.18，这俩求得u和v

    float hr  = fmaxf(gpr.x, 0.0f);//工公式里的h
    float hr4 = hr * hr * hr * hr;
    float vr  = sqrtf(2.0f) * hr * gpr.z / (sqrtf(hr4 + fmaxf(hr4, EPSILON)));//gpr.z这个是hv
    ...
    float a_plus  = fmaxf(fmaxf(vl + sqrtf(GRAVITY * (gpl.x /* + gpl.w*/)), vr + sqrtf(GRAVITY * (gpr.x /* + gpr.w*/))), 0);//这俩是a+和a-，在4.16和4.17
    ...
    float4 re = (a_plus * Fl - a_minus * Fr) / (a_plus - a_minus) + a_plus * a_minus / (a_plus - a_minus) * delta_U;//公式4.14，求通量
